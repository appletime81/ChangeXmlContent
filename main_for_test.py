...
def walkData(root_node, level, result_list):
    ...
    if (
        ...
        numDlSlotPart = root_node.text[
            re.search(f"<numDlSlot>\d+</numDlSlot>", root_node.text)
            .start() : re.search(f"<numDlSlot>\d+</numDlSlot>", root_node.text)
            .end()
        ]
        numDlSymbolPart = root_node.text[
            re.search(f"<numDlSymbol>\d+</numDlSymbol>", root_node.text)
            .start() : re.search(f"<numDlSymbol>\d+</numDlSymbol>", root_node.text)
            .end()
        ]
        numUlSlotPart = root_node.text[
            re.search(f"<numUlSlot>\d+</numUlSlot>", root_node.text)
            .start() : re.search(f"<numUlSlot>\d+</numUlSlot>", root_node.text)
            .end()
        ]
        numUlSymbolPart = root_node.text[
            re.search(f"<numUlSymbol>\d+</numUlSymbol>", root_node.text)
            .start() : re.search(f"<numUlSymbol>\d+</numUlSymbol>", root_node.text)
            .end()
        ]

        root_node.text = root_node.text.replace(
            numDlSlotPart, f"<numDlSlot>{NUMDLSLOT}</numDlSlot>"
        )
        root_node.text = root_node.text.replace(
            numDlSymbolPart, f"<numDlSymbol>{NUMDLSYMBOL}</numDlSymbol>"
        )
        root_node.text = root_node.text.replace(
            numUlSlotPart, f"<numUlSlot>{NUMULSLOT}</numUlSlot>"
        )
        root_node.text = root_node.text.replace(
            numUlSymbolPart, f"<numUlSymbol>{NUMULSYMBOL}</numUlSymbol>"
        )
